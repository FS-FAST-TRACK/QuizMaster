    ...
    public class QuestionDetailManager : IQuestionDetailManager
    {
        ...
        public async Task<bool> AddQuestionDetailAsync(Question question, IEnumerable<QuestionDetailCreateDto> questionDetailCreateDtos)
        {
            try
            {
                ...
                if (question.QType.Id == QuestionTypes.PuzzleSeedDataId)
                {
                    var optionIds = new List<int>(questionDetailTypes.Select(qD => qD.QuestionDetail.Id));

                    var answerQuestionDetail = new QuestionDetailType
                    {

                        QuestionDetail = new QuestionDetail()
                        {
                            QDetailDesc = JsonConvert.SerializeObject(optionIds),
                            Question = question
                        },
                        DetailTypeId = DetailTypes.keyValuePairs["answer"]
                    };
                    await _quizRepository.AddQuestionDetailTypeAsync(answerQuestionDetail);
                }
                return true;
            }
            catch (Exception ex)
